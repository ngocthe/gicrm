    ...
    <div class="custom-table">
      <client-only>
        ...
        <el-table
          ...
          border
        >
          <el-table-column min-width="100px" :label="$t('average')">
            <template #default="scope">
              {{
                priceDisplay(formatPriceFromMath(scope.row.average, 2)) +
                $t("yen")
...
import { getEndDate } from "~/../../src/shared/utils";
...
const getNameCsv = async () => {
  if (formData.mode === TYPE_WEEK) {
    return "A005_ダッシュボード画面_売上比率_週別";
  } else if (formData.mode === TYPE_DAY) {
    return "A005_ダッシュボード画面_売上比率_日別";
  } else {
    return "A005_ダッシュボード画面_売上比率_月別";
  }
};

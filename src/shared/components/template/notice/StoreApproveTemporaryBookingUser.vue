<template>
  <div v-if="isTitle" class="title-notice">
    ...
    <span
      ...
      >{{ bookingNumber }}</span
    >
    <span>{{ $t("template_notice.reservation_approved") }}</span>
  </div>
  <div v-else class="body-notice">
    ...
    <p>{{ $t("template_notice.following_reservation") }}</p>
    <br />
    ...
    <br />
    <p>{{ $t("template_notice.please_proceed_from") }}</p>
    <NuxtLink :to="bookingUrl" target="_blank">{{ bookingUrl }}</NuxtLink>
    <br />
    <p>
